...
async function createAssignments(inventoryData: InventoryItem[], people: any[], templates: any[]) {
  console.log('🔗 Creating assignments...')
  
  const createdAssignments = []
  
  for (const item of inventoryData) {
    if (!item['Nom du collaborateur'] || !item['Équipement'] || item['Équipement'] !== 'PC') {
      continue
    }
    ...
    let template = null
    if (item['Marque / Modèle']?.includes('HP 440 G8')) {
      template = templates.find(t => t.name.includes('HP ProBook'))
    } else if (item['Marque / Modèle']?.includes('M4 Air 13')) {
      template = templates.find(t => t.name.includes('MacBook'))
    } else if (item['Marque / Modèle']?.includes('ThinkPad')) {
    ...
    if (template) {
      ...
      console.log(`✅ Created assignment: ${person.name} -> ${item['Marque / Modèle']}`)
    } else {
      console.log(`⚠️ No template found for: ${item['Marque / Modèle']}`)
    }
  }
...
async function main() {
  try {
    ...
    const people = await createPeople(inventoryData)
    const templates = await createPackageTemplates(laptops, accessories)
    const assignments = await createAssignments(inventoryData, people, templates)
